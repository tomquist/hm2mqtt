...
- All meters (CT002 & SMR): Add *Refresh*, *Factory Reset* and *Hardware Reset* buttons (`cd=1`, `cd=11` and `cd=8`), bringing the meters in line with the B2500, Venus and Jupiter devices. All three are disabled by default.
- CT002: Poll `cd=19` for the per-phase charge and discharge counters (`ca`/`cb`/`cc` and `da`/`db`/`dc`), published on a new `.../phase_energy` topic as *Phase 1/2/3 Charge* and *Phase 1/2/3 Discharge*. These are not part of the `cd=1` payload, which is why they were previously invisible. Their unit could not be established, so the raw reported value is published with no unit or device class — if you can correlate them with your meter's readings, please report what you find. Polled every 5 minutes at most and disabled by default.
- Support the `TPM-CN` and `TPM2-X` device types (the Marstek CT002-CN and TPM2-100CT smart meters). They are sold under the same CT002 model as `HME-4` and speak the same protocol, so they get the full set of CT002 entities. The only difference is the phase-direction command, which spells its parameter `dir` instead of `p1` on `TPM2`.
- Support the `SMR-X` device type: the Marstek smart meter readers (`SMR-0` P1 Meter, `SMR-1` Infrared Meter, `SMR-2` TIC Meter), sold as the Marstek CT003 Smart Meter Reader. They report the same per-phase and total power as the CT002 smart meter, plus a *Total Energy* sensor (`eng_t`, reported in 0.1 Wh), a *P1 Device Connected* binary sensor (`har_f`) and the *Meter Number*, *P1 Read Status*, *Infrared Read Status* and *Phase Read Status* diagnostic sensors, which are disabled by default (fixes #379)
- CT002 & SMR: Expose the per-phase measurement direction (decoded from the `cur_d` bitmask, which flags the phases whose measurement direction is inverted) and add a *Slave Count* sensor (`slv_n`). Both device types report these keys. All new entities are disabled by default, and the measurement-direction entities only appear once the device has reported `cur_d`.
- CT002: The *Phase 1/2/3 Measurement Reversed* entities are settable switches, mirroring the *Reverse Measurement Direction* screen in the Marstek app. Because the device takes all three phases as one bitmask in a single `cd=5` command, toggling one switch re-sends the direction currently reported for the other two. On the SMR readers the same entities stay read-only binary sensors: they offer no measurement-direction setting, and their `cd=5` configures the attached smart meter instead.
