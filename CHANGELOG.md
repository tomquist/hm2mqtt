...
- MQTT Proxy: Stop renaming a device that is merely reconnecting. When a device reconnected while the proxy still held its previous, half-open session, the built-in proxy renamed the incoming client ID ("conflict resolution") instead of letting the new connection take the old session over. The stale session stayed alive until its keepalive expired, control messages were sent to it as well, and every reconnect added another ghost session — a constant stream of renames and reconnects in the log, even with devices that already use distinct client IDs. Renaming now only happens when the connections come from different addresses, i.e. when two different devices really do share one client ID, as B2500 firmware 226.5/108.7 does (fixes #398)
- MQTT Proxy: Fix client ID bookkeeping. A connection that was rejected or died during the MQTT handshake left its client ID marked as in use forever, so that device got renamed on every later connect; and after a session takeover the ID could be dropped from the tracking set although the new connection was using it.
