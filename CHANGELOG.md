...
- Note: if Home Assistant logs `Invalid option for select.…_meter_type` or `Exception in _message_received` after updating, reload the MQTT integration once (Settings → Devices & Services → MQTT → ⋮ → Reload) or restart Home Assistant (see #418)
