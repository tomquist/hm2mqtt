...
- Note: if Home Assistant logs `Invalid option for select.…_meter_type` or `Exception in _message_received` after updating, restart Home Assistant once. Reloading the MQTT integration is not enough — it silences the first error but leaves the second one running until a restart (see #418)
